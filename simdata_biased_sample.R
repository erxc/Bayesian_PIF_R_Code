...
simdata = function(n){
  ...
  Gamma_0 = matrix(c(0.2, 0.6, 0.2, -0.7,
  ...
  # observed data
  phi_0 = -4.5 #-2, -2.8, -3.5, -4.5
  phi = c(1, -0.5)
  R_prob = plogis(phi_0 + Z %*% phi)
  R = rbinom(n, 1, R_prob)
  data_full_1 = data.frame(Y=Y, X=X, Z=Z, U=U, R=R)
  data_1 = data_full_1
  data_1[R==0, c("X.1","X.2")] = NA
  data_full = rbind(data_full_1[R==0,], data_full_1[R==1,])
  data = rbind(data_1[R==0,], data_1[R==1,])
