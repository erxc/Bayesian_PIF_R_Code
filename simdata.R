...
simdata = function(n_m, n_v){
  ...
  Gamma_0 = matrix(c(-0.2, 0.4, 0.1, -0.7,  # high measurement error (correlation between X and Z)
                     0.1, 0.1, 0.25, -0.5), nr = d_x, nc = 1+d_x+d_u, byrow = TRUE)
  # Gamma_0 = matrix(c(-0.2, 1.5, 0.8, -0.7,  # low measurement error (correlation between X and Z)
  #                    0.1, 0.8, 1.2, -0.5), nr = d_x, nc = 1+d_x+d_u, byrow = TRUE)
  ...
  X = mu_x0 + rmvnorm(n = n, mean = rep(0, d_x), sigma = Sigma_x0)
